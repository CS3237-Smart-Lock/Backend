...
def insert_user():
    try:
        ...
        if len(faces) == 0:
            return {
                "error": "No faces detected, please ensure that the image is clear and the face is not hidden."
            }, 401

        if len(faces) > 1:
            return {
                "error": "More than one face detected. Please ensure that only one face is present in the image"
            }, 402

        image_with_face_highlighted = detector.get_image_with_face_circled(image_blob)

        base64_image = (
            f"data:image/jpeg;base64,{base64.b64encode(image_blob).decode('utf-8')}"
        )
